...
export type IdentityModeProps = Pick<
  HeroSection,
  | 'portrait'
  | 'monogram'
  | 'cardGreeting'
...
export function IdentityMode({
  ...
  return (
    <>
      ...
      <div data-mode-item className="absolute top-5 left-5 max-md:hidden">
        <Monogram value={monogram} />
      </div>

      {orderedSocials.length > 0 && (
        <ul data-mode-item className="absolute top-5 right-5 flex flex-col gap-2.5">
      ...
      {showBadge && (
        <div
          data-mode-item
          className="absolute top-[44%] -left-5 flex w-12 -translate-y-1/2 flex-col items-center gap-3 rounded-full border border-border bg-surface-raised/90 py-4 pl-5"
        >
      ...
      <div data-mode-item className="absolute inset-x-0 bottom-0 p-5">
        <h2 className="flex items-center font-display text-2xl font-semibold text-fg">
          {cardGreeting}
          <span aria-hidden="true" className="ml-1.5 inline-block h-6 w-0.5 animate-caret bg-fg" />
        </h2>
